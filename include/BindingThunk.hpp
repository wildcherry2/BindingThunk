...
/** @brief Expert-only overload that accepts a raw @c asmjit::FuncSignature directly for argument callbacks.
 ...
 *  @param ToFn Target function whose first parameter receives @p BindParam.
 *  @param BindParam Pointer bound into the first target argument. It must remain valid for the thunk lifetime.
 *  @param SourceSignature ABI signature exposed by the generated thunk. It must match @p ToFn without the bound parameter.
 *  @param Type Binding mode. This overload accepts @ref EBindingThunkType::Default and @ref EBindingThunkType::Register.
 *  @param bLogAssembly When true, emits generated assembly through the configured logger.
 *  @return Owning thunk pointer on success, or a detailed error on failure.
 */
...
/** @brief Typed wrapper for binding thunk generation using compile-time ABI deduction.
 ...
 *  @return Owning thunk pointer on success, or a detailed error on failure.
 */
THUNK_API FThunkResult GenerateBindingThunk(void(*ToFn)(void*, ArgumentContext&), void* BindParam, const ABISignature& SourceSignature, EBindingThunkType Type = EBindingThunkType::Argument, bool bLogAssembly = false);

/** @brief Typed wrapper for argument-context binding thunk generation using compile-time ABI deduction.
 *  @tparam BindParamType Type of the bound context pointer.
...
THUNK_API FThunkResult GenerateBindingThunk(void(*ToFn)(BindParamType*, ArgumentContext&), BindParamType* BindParam, EBindingThunkType Type = EBindingThunkType::Argument, const bool bLogAssembly = false) {
    ...
    return GenerateBindingThunk(reinterpret_cast<void(*)(void*, ArgumentContext&)>(ToFn), BindParam, SourceSignature.value(), Type, bLogAssembly);
}
